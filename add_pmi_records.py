...
def create_fake_patient_data(fake):
    ...
    return {
        ...
        'address_line_2': fake.secondary_address() if fake.boolean(chance_of_getting_true=25) else None,
        'town_or_city': fake.city(),
        'county': fake.county(),
        'postcode': fake.postcode(),
        'phone_mobile': fake.phone_number(),
        'phone_home': fake.phone_number() if fake.boolean(chance_of_getting_true=50) else None,
    ...
    try:
        ...
        for patient in patients_data:
             try:
                 ...
                 print(f"  Error inserting record for {patient.get('nhs_number', 'N/A')}: {e}")
                 conn.rollback()
        conn.commit()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Add fake patient records to the PMI database.")
    parser.add_argument("num_records", type=int, help="The number of fake records to create.")
