...
def find_patient_in_pmi(criteria):
    ...
    * MODIFIED to not flash messages *
    ...
    try:
        ...
        flash(f"DB search error: {e}", "error"); # This flash is OK (real error)
        patient = False
    finally:
        if cur: cur.close()
        if conn: conn.close()
...
def search_my_records():
    ...
    if form.validate_on_submit():
        ...
        steps_list = [
            ['You', 'PMI Database', '1. Search patient master index for my details...', 'request'],
            ['PMI Database', 'EHR Manager', '2. Look for an associated Electronic Health Record...', 'request'],
            ['You', 'EHR Manager', '3. EHR Manager sends back any records it found...', 'response']
        ]
        ...
        if patient_details and isinstance(patient_details, dict):
            ...
            if pmi_nhs:
                ...
                if ehr_id_res is False: print(f"Patient: API error check EHR {pmi_nhs}")
                    ...
                    flash("Your patient record was found, but no associated Electronic Health Record exists yet.", "info"); print(f"Patient: No EHR for {pmi_nhs}")
                    final_msg = "PMI record found. No EHR exists." # More specific message
                else:
                    ...
                    if comps_res:
                        ...
                        flash("Your Electronic Health Record was found, but it contains no entries yet.", "info"); print(f"Patient: No comps for {ehr_id}")
                        final_msg = "EHR found, but it is empty." # More specific message
            else: 
                flash("PMI record missing NHS number, cannot check EHR.", "warning")
                final_msg = "PMI record found, but has no NHS Number." # More specific message
