...
"""
Admin blueprint for the RTFM-EHR application.

Handles administrative routes, user management, and system configuration.
"""
...
def get_db_connection():
    ...
    if not os.path.exists(config_path):
        flash("Configuration file (config.ini) not found.", "error")
        return None
    config.read(config_path)
    if "pmi_database" not in config:
        flash("Missing [pmi_database] section in config.ini.", "error")
        return None
...
def create_fake_patient_data(fake):
    ...
    return {
        ...
        "address_line_2": fake.secondary_address() if fake.boolean(25) else None,
        "town_or_city": fake.city(),
        "county": fake.county(),
        "postcode": fake.postcode(),
        "phone_mobile": fake.phone_number(),
        "phone_home": fake.phone_number() if fake.boolean(50) else None,
    ...
    try:
        ...
        for patient in patients_data:
            try:
                ...
                print(f"DB Error inserting {patient.get('nhs_number')}: {e}")
                conn.rollback()
                skipped_count += 1
        conn.commit()
        if skipped_count > 0:
            flash(f"Skipped {skipped_count} records due to errors.", "warning")
    except psycopg2.Error as e:
        flash(f"DB error during batch: {e}", "error")
        conn.rollback()
        return 0
    finally:
        if cur:
        ...
        if conn:
            ...
            try:
                ...
                cur.execute(
                    sql,
                    (
                        form.nhs_number.data,
                        form.title.data or None,
                        form.given_name.data,
                        form.family_name.data,
                        form.date_of_birth.data,
                        form.gender.data or None,
                        form.address_line_1.data or None,
                        form.address_line_2.data or None,
                        form.town_or_city.data or None,
                        form.county.data or None,
                        form.postcode.data or None,
                        form.phone_mobile.data or None,
                        form.phone_home.data or None,
                        form.email_address.data or None,
                    ),
                )
                conn.commit()
                flash(
                    f"Patient {form.given_name.data} {form.family_name.data} added!",
                    "success",
                )
                return redirect(url_for("admin.add_pmi"))
            except psycopg2.errors.UniqueViolation:
                conn.rollback()
                flash(f"Error: NHS Number {form.nhs_number.data} exists.", "error")
            except psycopg2.Error as e:
                conn.rollback()
...
def batch_pmi():
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            if inserted > 0:
                flash(f"Added {inserted} batch records!", "success")
            elif not get_flashed_messages(category_filter=["error"]):
                flash("No new records added (duplicates?).", "info")
            return redirect(url_for("admin.batch_pmi"))
        except Exception as e:
            flash(f"Error generating/inserting: {e}", "error")
...
def view_create_ehr():
    form = SearchPmiForm()
    ehr_id, searched_nhs, action = None, None, None
    if form.validate_on_submit():
        ...
        if result is False:
            ...
            flash(f"No EHR for {nhs}. Creating...", "info")
            print(f"Creating EHR for {nhs}")
            new_id = ehrbase_api.create_ehr(nhs)
            if new_id:
                ehr_id, action = new_id, "created"
                flash(f"Created EHR: {ehr_id}", "success")
                print(f"Created {ehr_id}")
            else:
                action = "creation_failed"
                print(f"Failed create for {nhs}")  # Error flashed
        else:
            ehr_id, action = result, "found"
            flash(f"EHR exists: {ehr_id}", "info")
            print(f"Found {ehr_id}")
    return render_template(
        "view_create_ehr.html",
        title="View/Create EHR",
        form=form,
        ehr_id=ehr_id,
...
def sync_pmi_ehr():
    ...
    cur = None
    total, found, created, pmi_err, api_err = 0, 0, 0, 0, 0
    try:
        cur = conn.cursor()
        cur.execute(
            "SELECT nhs_number FROM patient_index WHERE nhs_number IS NOT NULL AND nhs_number != '';"
        )
        records = cur.fetchall()
        total = len(records)
        print(f"Found {total} records in PMI.")
        if total == 0:
            flash("No records in PMI.", "info")
            return redirect(url_for("admin.home"))
        for rec in records:
            ...
            if result is False:
                ...
                if new_id:
                    ...
                    api_err += 1
                    print(f"    Create failed for {nhs}.")
            else:
                found += 1
                print(f"    Exists: {result}")
    except psycopg2.Error as e:
        flash(f"DB error fetching records: {e}", "error")
        pmi_err = total
        print(f"DB error: {e}")
    except Exception as e:
        flash(f"Unexpected sync error: {e}", "error")
        api_err = total - found - created
    ...
    if pmi_err > 0:
        summary += f"PMI Errors: {pmi_err}. "
    if api_err > 0:
        summary += f"API Errors: {api_err}."
    ...
    if form.validate_on_submit():
        ...
        if conn:
            ...
            try:
                cur = conn.cursor()
                cur.execute(
                    "DELETE FROM patient_index WHERE nhs_number = %s", (nhs_number,)
                )
                conn.commit()
                if cur.rowcount > 0:
...
# --- End of app/blueprints/admin.py ---